...
/**
 * 
 * @param {string[]} input array of file paths
 * @param {string} pattern Glob pattern
 * @param {boolean} windows Force windows
 * @returns 
 */
function match(input, pattern, windows) {
  // required windows tweaks to keep compat with multimatch:
  // multimatch converts backslashes to forward slashes
  // micromatch only returns forward slashes in paths, hence the normalization formatting
  if (windows) {
    pattern = pattern.replace(/\\/g, '/');
  }
  return micromatch(input, pattern, {
    windows,
    format(p) {
      return normalize(p)
    }
...
const defaults = {
  ...
  matchingfiles: function (paths) {
    return match(paths, this.pattern);
  },
  indexpath: function () {
