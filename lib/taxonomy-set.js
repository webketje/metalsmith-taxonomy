 ...
 * @type Object
 *
 * @property {string|string[]} pattern One or more glob patterns supported by [multimatch](https://github.com/sindresorhus/multimatch) for the files to include in this taxonomy set
...
var defaults = {
  ...
  matchingfiles: function (paths) {
    return multimatch(paths, this.pattern);
  },
  indexpath: function () {
    return (this.namespace ? this.namespace : 'index') + '.html';
  },
  taxonomypath: function (taxonomy) {
    return path.join((this.namespace ? this.namespace + path.sep : '') + taxonomy + '.html');
  },
  termpath: function (term, taxonomy) {
    return path.join((this.namespace ? this.namespace + path.sep : '') + taxonomy, term + '.html');
  },
